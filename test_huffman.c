...
void test_huffman(const char *message)
{
    ...
    assert(status == 0);
    char *bit_header = malloc((encoded_message.header.nbits + 1) * sizeof(char));
    display_bit_message(&encoded_message.header, bit_header);
    printf("HEADER: %s\n", bit_header);
    free(bit_header);
    char *bit_message = malloc((encoded_message.message.nbits + 1) * sizeof(char));
    display_bit_message(&encoded_message.message, bit_message);
